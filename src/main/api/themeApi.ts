...
export async function handleThemeApi(opts: {
  ...
  if (method === 'POST' && path === '/api/v1/themes/install-zip') {
    try {
      ...
      if (st.builtin !== true) {
        ...
        if (!gate.ok) {
          try {
            await tm.uninstall(st.identifier)
          } catch {
            /* ignore */
          }
          sendJson(res, 403, {
            ...
            genuine: false,
            code: gate.code,
            message: `非正版授权，无法安装主题：${gate.message}`
          })
          return true
        }
      }
  ...
  if (method === 'POST' && enable) {
    try {
      ...
      const pack = tm.get(id) || tm.list().find((t) => t.identifier === id)
      if (pack && pack.builtin !== true) {
        const { resolve } = await import('path')
        const dataRoot = resolve(process.env.DATA_ROOT || `${process.cwd()}/data`)
        const { assertUsableIfMarketPack } = await import('../license/licenseGate')
        const gate = await assertUsableIfMarketPack({
        ...
        if (!gate.ok) {
          sendJson(res, 403, {
            ...
            genuine: false,
            code: gate.code,
            message: `非正版授权，无法启用主题：${gate.message}`
          })
          return true
        }
      }
